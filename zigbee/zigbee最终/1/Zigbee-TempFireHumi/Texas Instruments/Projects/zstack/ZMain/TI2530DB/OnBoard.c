...
void InitBoard( byte level )
{
  ...
  else  // !OB_COLD
  {
    ...
    OnboardKeyIntEnable = HAL_KEY_INTERRUPT_DISABLE;    //初始化为查询方式
...
{
  keyChange_t *msgPtr;
//按键事件被注册在sampleAPP应用registeredKeysTaskID = SampleApp_TaskID
  if ( registeredKeysTaskID != NO_TASK_ID )
  {
...
void OnBoard_KeyCallback ( uint8 keys, uint8 state )
{
  ...
  shift = (OnboardKeyIntEnable == HAL_KEY_INTERRUPT_ENABLE) ? false : (((keys & HAL_KEY_SW_6)||(keys & HAL_KEY_SW_5)) ? true : false);

  if ( OnBoard_SendKeys( keys, shift ) != ZSuccess )
  {
    // Process SW1 here
    if ( keys & HAL_KEY_SW_1 )  // Switch 1   _UP
    {
    }
    // Process SW2 here
    if ( keys & HAL_KEY_SW_2 )  // Switch 2   _RIGHT
    {
    }
    // Process SW3 here
    if ( keys & HAL_KEY_SW_3 )  // Switch 3   _DOWN
    {
    }
    // Process SW4 here
    if ( keys & HAL_KEY_SW_4 )  // Switch 4   _LEFT
    {
    }
    // Process SW5 here
//  if ( keys & HAL_KEY_SW_5 )  // Switch 5   _CENTER
    {
    }
    // Process SW6 here
